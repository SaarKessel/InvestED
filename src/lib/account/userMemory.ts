...
export function createLocalUserMemory(storage:Storage):UserMemoryStore{return{list:<T>(userId:string,scope?:MemoryScope)=>parse(storage).filter(r=>r.userId===userId&&(!scope||r.scope===scope)) as MemoryRecord<T>[],put:(record)=>{const all=parse(storage).filter(r=>!(r.userId===record.userId&&r.id===record.id));storage.setItem(KEY,JSON.stringify([...all,record]))},remove:(userId,id)=>storage.setItem(KEY,JSON.stringify(parse(storage).filter(r=>!(r.userId===userId&&r.id===id)))),clear:(userId,scope)=>storage.setItem(KEY,JSON.stringify(parse(storage).filter(r=>r.userId!==userId||(scope&&r.scope!==scope))))}}
