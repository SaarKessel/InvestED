...
export interface NewsEvent extends RawNewsItem{id:string;symbols:string[];eventType:EventType;classificationSource:"deterministic_rules";sentiment:null;sentimentReason:"not_configured"}
...
export function normalizeNews(item:RawNewsItem,knownSymbols:string[]):NewsEvent|null{if(!item.title.trim()||!/^https?:\/\//.test(item.url)||!item.source.trim()||Number.isNaN(Date.parse(item.publishedAt)))return null;const text=`${item.title} ${item.summary??""}`;let symbols=knownSymbols.filter(s=>new RegExp(`\\b${s.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\b`,"i").test(text));const extra=new Set<string>();for(const m of text.matchAll(/\(([A-Z]{1,5})\)|\$([A-Z]{1,5})\b/g)){const sym=m[1]??m[2];if(sym&&!symbols.includes(sym))extra.add(sym);}symbols=[...symbols,...extra];const eventType=rules.find(([,re])=>re.test(text))?.[0]??"unknown";return{...item,id:`${item.source}:${item.publishedAt}:${item.title}`.toLowerCase(),symbols,eventType,classificationSource:"deterministic_rules",sentiment:null,sentimentReason:"not_configured"}}
