  ...
  return <div className="space-y-6">
    ...
    {research && <>
      <section className="rounded-2xl border border-border bg-card p-5 shadow-sm" aria-labelledby="asset-title">
        ...
        <dl className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">{[[t("research_previous"),research.quote.previousClose],[t("research_volume"),research.quote.volume],[t("research_status"),research.quote.marketStatus],[t("research_freshness"),research.provenance.freshness]].map(([label,value])=><div className="rounded-xl bg-muted/50 p-3" key={String(label)}><dt className="text-xs text-muted-foreground">{label}</dt><dd className="mt-1 font-bold" dir={typeof value === "number" ? "ltr" : undefined}>{value ?? unavailable(language)}</dd></div>)}</dl>
      </section>
